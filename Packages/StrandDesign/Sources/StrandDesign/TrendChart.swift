        ...
        Chart {
            ...
            if showsBars {
                ...
                ForEach(displayPoints) { p in
                    ...
                    // Coloured by SAMPLING the ramp at the bar's own value — so the colour ENCODES the
                    // value (a low bar reads red, a high bar green), the same rule the point marks use.
                    // A vertical bottom→top gradient per bar looked identical on every bar and encoded
                    // nothing (redesign bug §1).
