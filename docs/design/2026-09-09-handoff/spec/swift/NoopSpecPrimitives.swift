...
public struct NoopSpecButton: View {
    ...
    public var body: some View {
        ...
        .buttonStyle(NoopButtonStyle(radius: radius, fill: accent, bordered: kind.bordered))
    }
}
...
public struct NoopButtonStyle: ButtonStyle {
    ...
    public func makeBody(configuration: Configuration) -> some View {
        ...
        return configuration.label
            ...
            .overlay(shape.fill(Color.white.opacity(configuration.isPressed ? 0.10 : 0.07)))
            .overlay(
...
public struct NoopSpecTabBar: View {
    ...
    // HIT TESTING IS A WRAPPER BEHAVIOUR, AND IT WAS DOCUMENTED HERE WITHOUT BEING IMPLEMENTED.
    // §20's container line is `allowsHitTesting(false)` on the outer block and `true` on the bar
    // row: the floating container spans the full width and carries 26 pt of bottom padding, so
    // without the false it is an invisible lid over the last row of content on every home screen.
    ...
    public var body: some View {
        GeometryReader { geo in
            ...
            .allowsHitTesting(true)                     // … and the BAR takes them back
        }
        .frame(height: 58)                      // 46 + 6 + 6
        .padding(.horizontal, 14)
        .padding(.bottom, 26)
        .allowsHitTesting(false)                // the CONTAINER passes taps through …
    }
    ...
    private func item(_ tab: NoopSpecTabDestination, width: CGFloat) -> some View {
        ...
        return Button(action: tab.action) {
            HStack(spacing: 7) {
                tab.icon
                    .foregroundStyle(on ? onAccent : NoopSpecTokens.textDim)   // ICON takes the ink too
                if on {
                    Text(tab.label)
                        .noopText(NoopSpecType.Role.buttonLabel)
...
public struct NoopSpecOrb: View {
    ...
    private func orb(at now: Date) -> some View {
        ...
        let bpm = bpmOverride ?? pacer.bpm(at: now)
        ...
        .background {
            SurroundHitRegion()
                .contentShape(SurroundHitRegion())
        ...
        .background(
            GeometryReader { g in
                Color.clear
                    ...
                    .onChange(of: g.size.height) { _, h in height = h; if h > 0 { measured = true } }
            }
        )
...
public struct NoopAccentCard<Content: View>: View {
    ...
    public var body: some View {
        let shape = RoundedRectangle(cornerRadius: radius, style: .continuous)
        // A CSS gradient angle is measured clockwise from "to top"; SwiftUI's unit points are not.
        // 158° is very slightly past horizontal-and-down, so: top-leading-ish to bottom-trailing-ish.
