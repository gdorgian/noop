...
| File | What |
| --- | --- |
| `00-RULES.md` | **Start here.** **Thirteen** rules — the things that make a faithful build look cheap — plus the provenance legend (§0, which is not one of them), Dynamic Type reflow and the nine recurring states. §13 was added on 31 August, which is when the "twelve" this line used to say stopped being true. |
...
| `12-palette-bindings.md` | **Every `NoopPalette` property with its value**, `controlFill` in full, the two greens and which is which, and — read this first if the build fails — where the six Swift files have to live for those names to resolve. |
...
`design_handoff_8_september/Noop app - standalone (open me).html` — **this pack's** standalone, the
whole app in one offline
...
4. **Notification mirroring has no owner.** Not app code as far as anyone has established, and
   nothing about the transport is settled — which is why `notifs` ships `[later]` with no permission
   request and no controls (`44-act5-plumbing.md` §5.6). The app list also cannot be pre-populated:
   iOS will not enumerate installed apps. **Smart Alarm is unaffected** and keeps its closed-app
   claim.
