...
## 0a · The release data rule — and it governs every other rule here

**Every word, measurement and derived value shown in a release build must have a real data source or
a defensible calculation.** No exceptions for empty states, previews, screenshots, App Store
imagery, or "we will wire it later".

Prototype values may exist **only behind both `#if DEBUG` and the `--demo-seed` runtime flag** —
both, not either. `#if DEBUG` keeps a fixture out of the release binary; `--demo-seed` keeps it out
of a debug build handed to a reviewer, who has to see the real absent states because those are what
a new user sees. `NoopCharge.demoSeed(...)` is the pattern: it returns `nil` unless both hold, so
the fall-through is the honest path rather than the seeded one.

**Nothing in this list may leak into the release IPA:** a fake API key, an OCR result for a
photograph nobody took, a health or body-age score, a charge level, a pulse, a session measurement,
a strap battery percentage, a notification count. Where the value is absent the screen renders its
**specified absent state** — which every act spec carries, because the absent state is part of the
screen rather than a gap in it (`41-act2-day.md` §2.2 is the worked example, and
`NoopSpecTokens.ChargeEvidence` is its implementation).

This is one rule rather than a note on each screen because it is the same failure every time: a
plausible number is easier to ship than an honest blank, and a plausible number in a health app is
a lie about someone's body.
