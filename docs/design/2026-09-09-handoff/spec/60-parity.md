...
**And Rev 3 got one wrong itself, corrected 8 September:** Mind was filed `[gone]` when **mood is
the Journal's first input** — the *How was it* row on `night/journal`, five states, in the canonical
HTML. Only its *reporting* is missing. It is `[thin]`, in §B, and **§A is now one item: Beat
Rhythm.**
...
| Missing with no substitute | **1** — Beat Rhythm |
| Thinner, with something nearby | **12** |
...
| Sleep | The Sleep tab: hypnogram, night-detail tiles, stages vs typical, asleep-duration trend | Act 1 as **five** screens — `rest`, `why`, `debt`, `alarm`, and `tonight`, which the original has no equivalent of | [better] |
...
**One.** Rev 2 listed eleven. Nine of the eleven are in Part 2 as [closed], with their routes named;
...
### 2 · What the nine closed screens had to satisfy
...
**Twelve.** Rev 2 listed fifteen. Three are closed and now sit in Part 2 (the Charge drivers, the
confidence ladder, search); one moved the other way, into §A as `[gone]` — Beat Rhythm, because
`trends/rhythm` is a different measurement with a similar name. **Mind stays here**, and Rev 3's own
first pass had it wrong: the input is built.
...
| Mind | **The input is built** — mood is the Journal's *first* question: the *How was it* row on `night/journal`, five states, in the canonical HTML. What is missing is the **reporting** — its correlations against Charge, Rest and HRV have no screen, and `instrument/effects` does not carry it. Rev 3 filed this `[gone]`, which was wrong twice over: there is a screen, and it is an input the design kept | [thin] |
...
4. **One capability with no screen and no row** — Beat Rhythm. Part 3 §A. **Mind is not on this
   list**: mood is the Journal's first input, and what is missing is its correlation reporting,
   which is a `[thin]` in Part 3 §B and worth a decision of its own.
...
What they will not find: beat-to-beat rhythm, their mood plotted against Charge, Rest and HRV (the
mood question itself is in the Journal), a Watch app, a light theme, and
