...
It is about seventy lines of constants and it shares a **purpose** with the deleted file and
nothing else — not even the name. `AuraPalette` is gone; `NoopPalette` is a new file, and no
symbol is common to the two. What it contains, and only this:
...
- the metrics (four radii, `screenPadding`, `cardGap`)

**Not** the motion durations. `breathDuration` and `sheenDuration` were in an earlier draft of this
list and in `NoopPalette` itself, where the breath read 6 s while `NoopSpecMotion` read 16 — one
timing in two files, disagreeing. They live in `NoopSpecMotion` only (16 s / 4 s / 24 s), and
`NoopPalette` is colour and metrics with no motion in it at all (`10-tokens.md`,
