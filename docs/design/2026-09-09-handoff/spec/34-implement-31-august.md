...
**Build.** The + is **contextual — one action per act**, and the nine rows are in `30-routes.md`
§*The +*. Where the act has its own sheet (Act 1's journal, Act 2's log, Act 4's logged sheet, Act
5's add sheet) the + presents it **over the current screen without navigating**. Where it does not,
the + goes to that act's own add destination: `effort/pick`, `plumbing/record`, `goal/picker`,
`plumbing/history` — and in Act 7 it focuses the Ask field already on screen. **This supersedes the
31 August build note**, which sent every sheet-less act to Today with the log sheet up
that leaves the user to find the + again.

```
+  →  the act's own add action  (30-routes.md §The +)
     Acts 1 · 2 · 4 · 5   present the act's own sheet, no navigation
     Act 3  → effort/pick        Act 6 → plumbing/record
     Act 8  → goal/picker        Act 9 → plumbing/history
     Act 7  → focus the Ask field, in place
```
