...
**Tab bar**, You active — blush `#E08A9B`, ink `#2A0E14` (§10's hue table).
...
7. **Buzz for phone notifications** → `plumbing/notifs`. Value slot reads `[static]` *"designed ·
   not built yet"* — the screen is `[later]` (§5.6), so there is no live summary to show. The
   `[formula]` (`{n} of 8 apps · {pattern}` · `mirroring off` · `needs notification access`) is what
   it becomes when the ANCS path exists.
...
**New, and `[later]` — the whole screen.** Reached from `strap` and from Settings → *What may
interrupt you*. Header: back → `strap`, parent `Your strap` `[static]`.

### Status, before anything else on this page is built

**Notification mirroring is not implemented, and this screen must not pretend otherwise.** iOS hands
notifications to a paired accessory only over **ANCS**, which the strap does not consume today; that
is firmware work, unshipped and unscheduled. So:

- The screen stays reachable, because the design is decided and hiding it hides the decision.
- **It requests no permission.** Asking for notification access that nothing can consume is the
  dishonest version of this screen.
- **The master control is a `Later` chip where the toggle would be** — the §5.10 drawer convention.
- Everything below it renders at **38 % opacity, non-interactive**, and **stores nothing**.
- The row that leads here reads *"designed · not built yet"*.

Everything in *Controls* below is therefore the **decided design of an unbuilt screen** — a record
of a decision, not a build instruction for this release. **What ships is the Later state and nothing
else:** no permission request, no *saved to strap* state, no strap-battery cost, and no active
control on the page. The permission states in the next section are what it will need **if and when
someone owns the transport**, and naming that transport is their job, not this file's.

### Permission is a first-class state — when this ships
...
Do not hide it — the user should see what they are unlocking. **While the feature is `[later]`, that
dimmed state is the only state**, and no permission card is shown at all.

### Controls — the design, recorded, and **not built in this release**

1. **Mirror phone notifications** — master toggle, aura hue `[bound]`. **Ships as a `Later` chip**,
   so neither sub-line below is what the build shows today.
   Shipping sub-line `[static]`: *"Not available. Noop still buzzes for its own things — bedtime,
   sessions, battery."*
   If it is ever built, on `[static]`: *"On. Nothing while the list below is empty."* — and the cost
   is stated in **interruptions, never in strap battery**. No row on this screen may price itself in
   the strap's day, because nothing on this screen runs on the strap (§5.16's execution-site table).
...
2. **Which ones reach your wrist** — caption `[static]` + list card, eight rows.
   ...
   Below the card, a line. **While the screen is `[later]` it is `[static]`:** *"Nothing in this
   list reaches your wrist while mirroring is unbuilt. The defaults are the intent: chat and calls
   through, feeds and newsletters not."*
   When it ships it becomes `[formula]`, `subline` 11.5 `textQuiet`:
...
5. **Footnote**, verbatim `[static]`: *"Noop never reads what a notification says — only which app
   sent it, so it knows whether to buzz."*

   **The battery figure is gone and does not come back with a guess.** This footnote carried *"about
   four percent of the strap's day"* — a number for an unbuilt feature on an unwritten firmware
   path, which is exactly the kind of measurement `00-RULES.md` §0 forbids. When mirroring ships,
   whoever measures it writes the real figure, or the sentence stays as it is.
...
- Whatever the eventual transport, the allow-list and conditions would be **device configuration**
  rather than app state. **No such sync exists, so the "saved to strap" state does not ship**, and
  it is not a 12th state in this release. A screen that says *saved to strap* when nothing was sent
  is worse than a screen that says *Later*. If a transport is ever confirmed, the state arrives in
  the same change that ships it — and the release notes name it then, not now.
...
2. **Rename** and **Forget** per row; Forget's sub-line states the consequence `[static]`, and
   **Forget asks for a confirmation** — a sheet, two buttons, `[static]` copy. (Specified here,
   beside the behaviour. There is no app-wide confirmation count: the app now confirms forgetting a
   strap, deleting data, ending a live session, turning sensitive journal topics on, and saving a
   goal the data will not back.)
...
Header: back → `you`, parent `You` `[static]`. **Not `settings`** — `data` is reached from both
`you`'s hub card and Settings, and its chevron reads *You* in the source (`30-routes.md`:
`data → you`). This line said `settings` and was the last surviving copy of a route the map had
already corrected.
...
### What actually runs where, and what may be claimed

**Nothing on this screen may claim to work with Noop closed.** The strap reports; **Noop decides and
Noop buzzes it back**, so every row here needs the app running — the background is enough, and iOS
may hold it a minute. The earlier version of this page said the opposite, in the app's own voice.

| Execution site | What is there | What the copy may say |
| --- | --- | --- |
| **Strap firmware, confirmed** | the **Smart Alarm** only (`night/alarm`, Act 1) | *keeps working with Noop closed* |
| **The app, foreground or background** | everything on this screen: the tap actions, the ceiling, the coach, the passive three | *while Noop is running* — and iOS may delay it |
| **Not implemented** | notification mirroring (§5.6) | *Later*, with no control to touch |

**A row whose execution site nobody can confirm ships as a `Later` row, not as a toggle.** The rule
is one sentence: *no promise about a closed app without a firmware path someone has named.* And
because these run in the app rather than on the strap, **the sub-lines are priced in interruptions
rather than in strap battery** — one buzz, one nudge, one check-in.
...
**Footnote**, verbatim `[static]`: *"Everything here runs through Noop rather than on the strap:
the strap reports, the app decides, the app buzzes it back. Each one needs Noop running — the
background is enough, and iOS may hold it a minute. The one thing that keeps working with Noop
closed is the Smart Alarm, because that is armed in the strap's own firmware."*

**Implementation note** — this screen is **app configuration**, not device configuration, and there
is no *saved to strap* state on it. If any row later moves into firmware, it earns that state in the
same change that moves it — and only then may its sub-line mention a closed app.
