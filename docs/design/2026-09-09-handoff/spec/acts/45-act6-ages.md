...
**Tab bar** — all five wired, and **nothing is lit**: `ages` is not one of the five destinations,
so all four tabs sit at 1× and the width divides by 4 rather than 4.7 (§10). **`tabTrends` was
found dead in this act** — check it first.
...
Header: back → **`picture/trends`**, parent `Trends` `[static]`. `building` is entered from Trends,
not from `ages`, and its chevron says so — **the back leaves the act** (`30-routes.md`). Sending it
to `ages` puts the user somewhere they were never standing. Its tab bar therefore shows an active
**Trends** tab in the act's green.
...
Header: back → **`plumbing/you`**, parent `You` `[static]`. `health` is entered from You, and like
`building` **its back leaves the act** (`30-routes.md`). Its tab bar shows an active **You** tab in
amber.
