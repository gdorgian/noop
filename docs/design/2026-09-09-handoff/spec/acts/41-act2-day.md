...
**charge glow 268, charge core 176** (both absent below 3 % heat — they are how the sphere carries
the level), sheen 176. On the orb: pulse `heroNumeral` 52 in `#F6FDFF` `[bound]`, breathing word
`breathWord` 12.5 tracking +1.375 `[formula]`.
**No numeric readout under the orb.** This was tried and removed — if the build has one, delete it.
**Two tap targets:** the **Ø 176 sphere** → `day/breathe`; the **surround** (gauge, halo, block)
→ `day/charge`. Plain taps, and **no long press** — see `20-primitives.md` §11.
...
**Svea prompt** — the **158° directional accent card**, radius 20, padding 14 / 16:
`linear-gradient(158deg, rgba(139,153,214,.16), rgba(139,153,214,.03))` over a 0.5 pt
`rgba(139,153,214,.30)` border → `svea/coach`. **Not `coachSurfaceTop → coachSurfaceBottom`** —
those two tokens named a near-black vertical ramp that appears in no source and are deleted
(`10-tokens.md`). It is also not "the one exception to no gradients": that rule was false, and the
registry of what the app really uses is in `10-tokens.md` §*Gradients and shadows, as built*.
...
**Tab bar**, Today active — `flex 1.7`, filled `#17A2E6` with `#04121A` ink, icon + label. The
fill is the **act's** hue passed into the primitive, not `NoopPalette.accent` read inside it; on
this act the two happen to be the same colour, which is exactly how the bug hid (§10's hue table).
...
Reached by tapping the orb's **surround** — not the sphere, which opens Breathe — **or** the state
line. Not in the tab bar, not in a menu.
