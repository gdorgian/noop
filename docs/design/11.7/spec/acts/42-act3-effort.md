...
**Tab bar** — no tab is active in this act. All five still wired.
...
5. Two buttons, pinned to the bottom with `padding .bottom 26`: **Intervals** secondary →
   ...
   End requires a **confirm** — one of only two confirmations in the app besides disconnecting a
   strap and deleting data. Sheet, two buttons, `[static]` copy.
...
Header: back → `live`, parent `Live` `[static]`. Tab bar hidden.
