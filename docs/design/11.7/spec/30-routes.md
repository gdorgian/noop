...
| **+** | — | opens the log sheet over the current screen |
...
pass found it inert in Acts 6, 7 and 8 — the three acts with no sheet of their own. An act without a
log sheet routes the + to `day/today` **with the log sheet already up**; it never does nothing.
...
Eleven, in three groups. A screen on this list has no chevron in its header and no entry in its
act's map, and the two facts are the same fact.
...
| `today` | the orb, long press | `day/breathe` |
...
| `nAccess` — iOS notification authorisation | system | Read live on every appearance of `notifs`, not cached |
