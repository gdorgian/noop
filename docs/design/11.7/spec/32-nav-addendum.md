...
- The **+** still opens the log sheet. It does not become a session button.
- No long-press. Nothing in Noop is long-press-only.
...
1. **The + was inert in Acts 6, 7 and 8** — the three acts with no sheet of their own had a centre
   button with no handler at all, which is checklist item 3 failing from three of nine acts. Fixed:
   those acts route the + to `day/today` with the log sheet already open (`sheet: 'log'`, seq-driven
   so a second tap re-opens it). §30's tab rule now names the + as a tab.
