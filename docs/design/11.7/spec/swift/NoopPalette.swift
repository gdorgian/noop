...
// else — no components, no screens, no body-state type, no copy, no behaviour, and no gradient
// beyond the one orb ramp below.
//
...
public enum NoopPalette {
    ...
    /// The coaching card's gradient, which lifts it off the flat card fill so Svea's call reads as
    /// the screen's one instruction rather than one more panel. The app's ONLY card gradient.
    public static let coachSurfaceTop = srgb(24, 33, 30)     // #18211E
    public static let coachSurfaceBottom = srgb(18, 22, 21)  // #121615
    ...
    /// RETIRED FOR TYPE, 20 September 2026. 3.0:1 on surfaceBase — no size of type
    /// passes with it, and it was carrying 11 pt captions. Every caption that used it
    /// now uses textQuiet (#7F8A85, 5.5:1). The value survives under a new name for the
    /// row chevron only, which is a graphical object and fine at 3:1.
    public static let chevronDim = srgb(87, 96, 92)          // #57605C — NEVER for type
    ...
    /// One breath of the orb. Slow enough to read as breathing rather than as a pulse.
    public static let breathDuration: Double = 6
    /// One rotation of the orb's specular sheen. Deliberately incommensurate with `breathDuration`,
    /// so the two never resynchronise into a visible beat.
