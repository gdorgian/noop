...
final class LiveActivityController {
    ...
    private var liftSuppressed = false
    private var lastObservedBPM: Int?
    ...
    /// Drive the activity from actual samples. A disconnected strap leaves the activity visible with
    /// no figure; a Lift session replaces it with the dedicated Lift activity.
    func update(bpm: Int?, recovery: Int?, connected: Bool, effort: Int? = nil,
                zoneNumber: Int? = nil, ceilingBPM: Int? = nil,
                newReading: Bool = false, suppressedByLift: Bool = false) {
        ...
        let now = Date()
        if connected, newReading, let bpm, bpm > 0 {
            lastObservedBPM = bpm
            lastObservedAt = now
        }
        // Re-adopt the last timestamp from an activity that survived an app relaunch. Never stamp
        // a cached keep-alive value with the current time; that would make an old sample look fresh.
        if lastObservedAt == nil, let state = activity?.content.state {
            lastObservedBPM = state.lastBPM ?? state.bpm
            lastObservedAt = state.lastSampledAt ?? state.sampledAt
        }
        guard connected || activity != nil else { return }
        let fresh = connected && lastObservedAt.map { now.timeIntervalSince($0) < Self.staleAfter } == true
        let presentedBPM = fresh ? lastObservedBPM : nil

        let state = NOOPActivityAttributes.ContentState(
            bpm: presentedBPM, recovery: recovery, bonded: connected, effort: effort,
            sampledAt: presentedBPM == nil ? nil : lastObservedAt,
            lastBPM: lastObservedBPM, lastSampledAt: lastObservedAt,
            zoneNumber: presentedBPM == nil ? nil : zoneNumber,
            ceilingBPM: ceilingBPM
        )
        let staleDate = lastObservedAt?.addingTimeInterval(Self.staleAfter)
            ?? now.addingTimeInterval(Self.staleAfter)

        if let activity {
            guard now.timeIntervalSince(lastPush) > 2 || !connected else { return }
            lastPush = now
            ...
            do {
                ...
                lastPush = now
                // Lift may have started while Activity.request was in flight. Do not leave two
                // banners stacked merely because its first suppression tick saw no handle yet.
                if liftSuppressed { Task { await end() } }
            } catch {
                activity = nil
            }
            isStarting = false
