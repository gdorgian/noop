...
struct CoachSettingsView: View {
        ...
        NoopCard(padding: 14, tint: StrandPalette.chargeColor) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                .onChangeCompat(of: briefEnabled) { on in
                    CoachBriefScheduler.setEnabled(on, generateBrief: { await coach.generateBriefText() }) { outcome in
                        if outcome == .denied {
                            briefEnabled = false
    ...
    private var briefTimeBinding: Binding<Date> {
        Binding(
            ...
            set: { date in
                ...
                briefMinutes = m
                CoachBriefScheduler.setTimeMinutes(m, generateBrief: { await coach.generateBriefText() })
            }
        )
    }
    ...
    private func generateBriefNow() {
        Task {
            ...
            let text = await CoachBriefScheduler.generateNow { await coach.generateBriefText() }
            if let text {
